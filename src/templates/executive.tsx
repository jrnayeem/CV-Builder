...
const ExecutiveLayout: React.FC<TemplateProps & { primaryColor: string; accentColor: string }> = ({ data, primaryColor, accentColor }) => (
  <div className="w-[210mm] h-[297mm] flex flex-col bg-white text-left font-serif" style={{ fontFamily: "Georgia, serif" }}>
    <header className="px-16 pt-16 pb-8 border-b-4 flex justify-between items-end" style={{ borderColor: primaryColor }}>
      <div>
        <h1 className="text-4xl font-bold uppercase tracking-wide" style={{ color: primaryColor }}>{data.name || 'Your Name'}</h1>
      ...
      <div className="grid grid-cols-2 gap-12">
        <div className="flex flex-col gap-8">
          {data.experience.length > 0 && (
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ color: primaryColor, borderColor: accentColor }}>Experience</h2>
          ...
          <CustomSectionsBlock data={data} headingColor={primaryColor} borderColor={accentColor} headingSize="text-lg font-bold uppercase tracking-widest" />
        </div>
        <div className="flex flex-col gap-8">
          {data.education.length > 0 && (
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ color: primaryColor, borderColor: accentColor }}>Education</h2>
              <div className="space-y-4">
                {data.education.map(edu => (
                  <div key={edu.id}>
                    <h3 className="font-bold text-gray-900">{edu.degree}</h3>
                    <div className="text-sm font-semibold" style={{ color: accentColor }}>{edu.institution}</div>
          ...
          {data.skills.length > 0 && (
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ color: primaryColor, borderColor: accentColor }}>Core Competencies</h2>
              <div className="grid grid-cols-2 gap-2">
                {data.skills.map(s => (
                  <div key={s.id} className="text-xs font-medium text-gray-700 flex items-center gap-2">
                    <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: accentColor }}></span>
                    {s.name}
                  </div>
                ))}
              </div>
          ...
          {data.languages.length > 0 && (
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ color: primaryColor, borderColor: accentColor }}>Languages</h2>
          ...
          {data.references.length > 0 && (
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest mb-4 border-b pb-2" style={{ color: primaryColor, borderColor: accentColor }}>References</h2>
          ...
          <PersonalDetailsBlock data={data} headingColor={primaryColor} borderColor={accentColor} compact />
        </div>
      </div>
    </div>
  </div>

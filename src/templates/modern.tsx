...
const ModernSplitLayout: React.FC<TemplateProps & { leftColor: string; textColor: string; accentColor: string }> = ({ data, leftColor, textColor, accentColor }) => (
  <div className="w-[210mm] h-[297mm] flex bg-white text-left font-sans">
    <div className="w-[40%] p-8 text-white h-full flex flex-col gap-6" style={{ backgroundColor: leftColor }}>
      {data.photo && (
    ...
    <div className="w-[60%] p-10 text-gray-800 h-full flex flex-col gap-8">
      {data.objective && (
        <div>
          <h2 className="text-lg font-black uppercase tracking-widest mb-3" style={{ color: accentColor }}>Profile</h2>
          <p className="text-sm leading-relaxed text-gray-600">{data.objective}</p>
        </div>
      )}
      {data.experience.length > 0 && (
        <div>
          <h2 className="text-lg font-black uppercase tracking-widest mb-4" style={{ color: accentColor }}>Experience</h2>
          <div className="space-y-6">
            {data.experience.map(exp => (
              <div key={exp.id} className="relative pl-4 border-l-2" style={{ borderColor: accentColor }}>
                ...
                <div className="text-sm font-medium mb-2" style={{ color: textColor }}>{exp.company} | {exp.startDate} - {exp.endDate}</div>
      ...
      {data.education.length > 0 && (
        <div>
          <h2 className="text-lg font-black uppercase tracking-widest mb-4" style={{ color: accentColor }}>Education</h2>
          <div className="space-y-4">
            {data.education.map(edu => (
              <div key={edu.id} className="relative pl-4 border-l-2" style={{ borderColor: accentColor }}>
                <div className="absolute w-2 h-2 rounded-full -left-[5px] top-1.5" style={{ backgroundColor: accentColor }}></div>
                <h3 className="font-bold text-gray-900">{edu.degree}</h3>
                <div className="text-sm font-medium mb-1" style={{ color: textColor }}>{edu.institution} | {edu.year}</div>
      ...
      <CustomSectionsBlock data={data} headingColor={accentColor} borderColor={accentColor} compact headingSize="text-lg font-black uppercase tracking-widest" />
      <PersonalDetailsBlock data={data} headingColor={accentColor} borderColor={accentColor} compact />
    </div>
  </div>
);

const ModernHeaderLayout: React.FC<TemplateProps & { headerColor: string; textColor: string; isGradient?: boolean }> = ({ data, headerColor, textColor, isGradient = false }) => (
  <div className="w-[210mm] h-[297mm] flex flex-col bg-white text-left font-sans">
    <div className="w-full px-12 py-10 text-white flex justify-between items-center"
         style={isGradient ? { background: `linear-gradient(135deg, ${headerColor}, ${textColor})` } : { backgroundColor: headerColor }}>
    ...
    <div className="flex-1 p-12 grid grid-cols-12 gap-8 text-gray-800">
      <div className="col-span-8 flex flex-col gap-8">
            ...
            <div className="space-y-6">
              {data.experience.map(exp => (
                <div key={exp.id}>
                  ...
                  <div className="text-sm font-medium mb-2" style={{ color: headerColor }}>{exp.company}, {exp.location}</div>
                  <p className="text-sm text-gray-600 whitespace-pre-wrap">{exp.description}</p>
                </div>
              ))}
            </div>
        ...
        <CustomSectionsBlock data={data} headingColor={headerColor} borderColor="#e5e7eb" headingSize="text-sm font-black uppercase tracking-widest" />
      </div>
      <div className="col-span-4 flex flex-col gap-8">
          ...
          <div>
            ...
            <div className="space-y-4">
              {data.education.map(edu => (
                <div key={edu.id}>
                  <h3 className="font-bold text-gray-900">{edu.degree}</h3>
                  <div className="text-sm font-medium mb-1" style={{ color: headerColor }}>{edu.institution}</div>
              ...
              {data.skills.map(s => (
                <div key={s.id}>
                  ...
                  <div className="h-1 w-full bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${s.level * 20}%`, backgroundColor: headerColor }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <PersonalDetailsBlock data={data} headingColor={headerColor} borderColor="#e5e7eb" compact />
      </div>
    </div>
  </div>
);

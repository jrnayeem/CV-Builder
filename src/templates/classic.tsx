...
const ClassicLayout: React.FC<TemplateProps & { sidebarColor: string; textColor: string; accentColor: string; fontFamily?: string }> = ({ data, sidebarColor, textColor, accentColor, fontFamily = '"Times New Roman", Times, serif' }) => (
  <div className="w-[210mm] h-[297mm] flex bg-white text-left" style={{ fontFamily }}>
    <div className="w-[35%] p-8 text-white h-full flex flex-col gap-6" style={{ backgroundColor: sidebarColor }}>
      {data.photo && (
        <div className="w-32 h-32 rounded-full overflow-hidden border-4 mx-auto mb-4" style={{ borderColor: accentColor }}>
          <img src={data.photo} alt="Profile" className="w-full h-full object-cover" />
        </div>
      )}
      <div>
        <h2 className="text-xl font-bold border-b-2 pb-2 mb-4" style={{ borderColor: accentColor }}>Contact</h2>
      ...
      {data.skills.length > 0 && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-2 mb-4" style={{ borderColor: accentColor }}>Skills</h2>
      ...
      {data.languages.length > 0 && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-2 mb-4" style={{ borderColor: accentColor }}>Languages</h2>
      ...
      {data.hobbies && data.hobbies.length > 0 && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-2 mb-4" style={{ borderColor: accentColor }}>Hobbies</h2>
    ...
    <div className="w-[65%] p-8 text-gray-800 h-full flex flex-col gap-6">
      <div>
        <h1 className="text-4xl font-bold text-gray-900 mb-1 uppercase tracking-wider" style={{ color: textColor }}>{data.name || 'Your Name'}</h1>
      ...
      {data.experience.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold border-b-2 pb-2 mb-4 uppercase tracking-wide" style={{ borderColor: accentColor, color: textColor }}>Experience</h2>
      ...
      {data.education.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold border-b-2 pb-2 mb-4 uppercase tracking-wide" style={{ borderColor: accentColor, color: textColor }}>Education</h2>
          <div className="space-y-4">
      ...
      <CustomSectionsBlock data={data} headingColor={textColor} borderColor={accentColor} />
      {data.references.length > 0 && (
        <div>
          <h2 className="text-2xl font-bold border-b-2 pb-2 mb-4 uppercase tracking-wide" style={{ borderColor: accentColor, color: textColor }}>References</h2>
      ...
      <PersonalDetailsBlock data={data} headingColor={textColor} borderColor={accentColor} />
    </div>
  </div>
);

const ClassicHeaderLayout: React.FC<TemplateProps & { headerColor: string; accentColor: string }> = ({ data, headerColor, accentColor }) => (
  <div className="w-[210mm] h-[297mm] flex flex-col bg-white text-left font-serif" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
    <div className="w-full p-8 text-white text-center flex flex-col items-center" style={{ backgroundColor: headerColor }}>
      {data.photo && (
    ...
    <div className="flex-1 p-10 flex flex-col gap-6 text-gray-800">
      {data.objective && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-1 mb-3 uppercase tracking-wide" style={{ borderColor: accentColor, color: headerColor }}>Objective</h2>
          <p className="text-sm leading-relaxed">{data.objective}</p>
        </div>
      )}
      {data.experience.length > 0 && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-1 mb-4 uppercase tracking-wide" style={{ borderColor: accentColor, color: headerColor }}>Experience</h2>
          <div className="space-y-4">
            {data.experience.map(exp => (
              <div key={exp.id}>
      ...
      {data.education.length > 0 && (
        <div>
          <h2 className="text-xl font-bold border-b-2 pb-1 mb-4 uppercase tracking-wide" style={{ borderColor: accentColor, color: headerColor }}>Education</h2>
      ...
      <div className="grid grid-cols-2 gap-8">
        {data.skills.length > 0 && (
          <div>
            <h2 className="text-xl font-bold border-b-2 pb-1 mb-3 uppercase tracking-wide" style={{ borderColor: accentColor, color: headerColor }}>Skills</h2>
            <div className="space-y-2">
              {data.skills.map(s => (
                <div key={s.id}>
                  <div className="flex justify-between text-sm mb-1"><span>{s.name}</span></div>
                  <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full rounded-full" style={{ width: `${s.level * 20}%`, backgroundColor: accentColor }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        {data.languages.length > 0 && (
          <div>
            <h2 className="text-xl font-bold border-b-2 pb-1 mb-3 uppercase tracking-wide" style={{ borderColor: accentColor, color: headerColor }}>Languages</h2>
      ...
      <CustomSectionsBlock data={data} headingColor={headerColor} borderColor={accentColor} />
      <PersonalDetailsBlock data={data} headingColor={headerColor} borderColor={accentColor} />
    </div>
  </div>
);

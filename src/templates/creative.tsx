...
const CreativeLayout: React.FC<TemplateProps & { bgColor: string; accentColor: string; gradient?: string }> = ({ data, bgColor, accentColor, gradient }) => (
  <div className="w-[210mm] h-[297mm] flex flex-col bg-white text-left font-sans">
    <div className="relative w-full h-48 overflow-hidden" style={gradient ? { background: gradient } : { backgroundColor: bgColor }}>
    ...
    <div className="px-12 pt-8 pb-4">
      <h1 className="text-5xl font-black tracking-tighter" style={{ color: bgColor }}>{data.name || 'Your Name'}</h1>
        ...
        {data.skills.length > 0 && (
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {data.skills.map(s => (
                <div key={s.id} className="px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm" style={{ backgroundColor: accentColor }}>
                  {s.name}
                </div>
              ))}
            </div>
              ...
              {data.languages.map(l => (
                <div key={l.id} className="text-sm font-bold text-gray-700 flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    {[1,2,3,4,5].map(i => (
                      <div key={i} className={`w-2 h-2 rounded-full ${i <= l.level ? '' : 'opacity-20'}`} style={{ backgroundColor: accentColor }}></div>
                    ))}
                  </div>
                </div>
              ))}
      ...
      <div className="w-2/3 px-12 py-4 flex flex-col gap-8">
        {data.objective && (
          <div className="relative">
            <div className="absolute -left-4 top-0 text-6xl opacity-10" style={{ color: bgColor }}>"</div>
            <p className="text-sm font-medium leading-relaxed text-gray-600 italic z-10 relative">{data.objective}</p>
          </div>
        )}
        {data.experience.length > 0 && (
          <div>
            <h2 className="flex items-center gap-4 text-xl font-black uppercase tracking-widest mb-6" style={{ color: bgColor }}>
              ...
              {data.experience.map(exp => (
                <div key={exp.id}>
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-sm font-bold" style={{ color: accentColor }}>{exp.company}</span>
        ...
        <CustomSectionsBlock data={data} headingColor={bgColor} borderColor="#e5e7eb" headingSize="text-xl font-black uppercase tracking-widest" />
        <PersonalDetailsBlock data={data} headingColor={bgColor} borderColor="#e5e7eb" compact />
      </div>
    </div>
  </div>
);

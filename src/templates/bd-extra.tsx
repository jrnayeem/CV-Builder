...
const BDColoredHeaderLayout: React.FC<TemplateProps & { headerColor: string; accentColor: string }> = ({ data, headerColor, accentColor }) => (
  <div className="w-[210mm] h-[297mm] flex flex-col bg-white text-left font-sans text-gray-900">
    <div className="w-full text-white px-10 py-6 flex justify-between items-center" style={{ backgroundColor: headerColor }}>
    ...
    <div className="px-10 py-5 flex-1 flex flex-col gap-4 overflow-hidden">
      {data.objective && (
        <div>
          <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>Career Objective</h2>
          <p className="text-xs text-justify leading-relaxed">{data.objective}</p>
        </div>
      )}
      {data.education.length > 0 && (
        <div>
          <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>Educational Qualification</h2>
      ...
      {data.experience.length > 0 && (
        <div>
          <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>Experience</h2>
      ...
      <CustomSectionsBlock data={data} headingColor={accentColor} borderColor={accentColor} compact headingSize="text-xs font-bold uppercase" />
      <div className="grid grid-cols-2 gap-6">
        <div>
          <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>Personal Details</h2>
        ...
        {data.skills.length > 0 && (
          <div>
            <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>Skills</h2>
      ...
      {data.references.length > 0 && (
        <div>
          <h2 className="text-xs font-bold uppercase py-0.5 mb-2 border-b" style={{ color: accentColor, borderBottomColor: accentColor }}>References</h2>

...
export function CvScoreDialog() {
  ...
  const handleScore = async () => {
    ...
    try {
      ...
      const body = await res.json().catch(() => ({})) as { error?: string; result?: ScoreResult; setup_required?: boolean };
      if (!res.ok) {
        if (body.setup_required) { throw new Error("__setup__"); }
            ...
            {error && !loading && (
              <div className="flex flex-col items-center gap-4 py-8">
                {error === "setup_required" ? (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-4 space-y-2 w-full">
